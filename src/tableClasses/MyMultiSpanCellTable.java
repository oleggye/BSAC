...
public class MyMultiSpanCellTable extends MultiSpanCellTable {
    ...
    public void changeTableModel(Record record) {
        int[] rows = new int[]{record.getSubjOrdinalNumber() - 1}; //т.к. в таблице индексы с 0
        int[] cols = SupportClass.getColsFromSubgroup(record.getSubjectFor().getId());

        AttributiveCellTableModel tableModel = (AttributiveCellTableModel) this.getModel();//задаем переменную с моделью текущей таблицы
        tableModel.setValueAt(record, rows[0], cols[0]);//задаем значение

        if (cols.length > 1) {//т.е. если нужно объединять ячейки, т.к. в качестве подгруппы "Вся группа"
    ...
    public void setDefaultAttributiveCellTableModel() {
        AttributiveCellTableModel tableModel = new AttributiveCellTableModel(
                new Object[][]{{1, null, null}, {2, null, null}, {3, null, null}, {4, null, null},
    ...
    public void setModelForTable(List<Record> mainRecordsCollection, ArrayPosition arrPosition) {
        ...
        this.setDefaultAttributiveCellTableModel();//задаем дефолтную модель для таблицы
        for (Record e : mainRecordsCollection) {
            if (e.getWeekDay() == (arrPosition.getColIndex() + 1) && e.getWeekNumber() == (arrPosition.getRowIndex() + 1)) {
