...
import timetable.util.UtilityClass;
...
public class UpdateOrCancelForm extends JDialog {
	 ...
	 */
	public UpdateOrCancelForm(JFrame parent, Date lessonDate, Group group, Record record, byte weekNumber,
	...
	private void constactUpdateTab(JPanel updatePanel, String lessonDate) {
		...
		JDatePickerImpl fromDatePickerForUpdate = initializer.initDatePicker(updateRecord.getDateFrom());
		...
		/*
		 * устанавливаем значения выпадающих списков согласно переданной записи
		 * (currentRecord)
		 */
		try {
			initializer.initChairComboBox(updateChairComboBox);
			...
			initializer.initClassroomComboBox(classroomComboBox);
	...
	private void constractCancelTab(JPanel cancelTab, String date) {
		...
		JDatePickerImpl fromDatePickerForCanceling = initializer.initDatePicker(cancelRecord.getDateFrom());
