		...
		addButton.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					ICommand command = CommandProvider.getInstance().getCommand(ActionMode.Add_Subject);
		...
		deleteButton.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int result = JOptionPane.showConfirmDialog(getContentPane(),
						"Убедитесь, что удаляемая дисциплина не связана с записями текущего расписания", "Внимание!",
						JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
				if (result == JOptionPane.YES_OPTION) {
					deleteButton.setEnabled(false);
					try {
						ICommand command = CommandProvider.getInstance().getCommand(ActionMode.Delete_Subject);
						Request request = new Request();
						request.putParam("subject", subject);
