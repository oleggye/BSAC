		...
		addButton.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Chair chair = (Chair) chairComboBox.getSelectedItem();
					Lecturer lecturer = new LecturerBuilder().buildChair(chair).buildNameLecturer(lecturerName).build();

					ICommand command = CommandProvider.getInstance().getCommand(ActionMode.Add_Lecturer);
					Request request = new Request();
					request.putParam("lecturer", lecturer);
		...
		editButton.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					ICommand command = CommandProvider.getInstance().getCommand(ActionMode.Update_Lecturer);
					Request request = new Request();
					request.putParam("lecturer", lecturer);
		...
		deleteButton.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int result = JOptionPane.showConfirmDialog(getContentPane(),
						"Убедитесь, что удаляемый преподаватель не связан с записями текущего расписания", "Внимание!",
						JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
				if (result == JOptionPane.YES_OPTION) {
					deleteButton.setEnabled(false);
					try {
						ICommand command = CommandProvider.getInstance().getCommand(ActionMode.Delete_Lecturer);
						Request request = new Request();
						request.putParam("lecturer", lecturer);
